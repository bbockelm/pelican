...
func NamespaceDelete(endpoint string, prefix string) error {
	...
	// // Namespace URL is also the issuer URL.
	// namespaceURLStr := viper.GetString("NamespaceURL")
	// if namespaceURLStr == "" {
	// 	return errors.New("Namespace URL is not set")
	// }

	// issuerURL, err := url.Parse(namespaceURLStr)
	// if err != nil {
	// 	return errors.Wrap(err, "Failed to parse namespace URL for deletion token creation")
	// }
